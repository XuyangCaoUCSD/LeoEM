...
# an array containing (lat, long, alt) tuples for all the ground stations
ground_station_llas = []
ground_station_cbfs = []
...
for current_cycle in range(1, cycles + 1):
    ...
    try:
        G = nx.Graph()
        edges = []
        G.add_nodes_from(range(satellite_num + len(ground_station_llas)))
        ...
        for satellite_id in range(0, satellite_num):
            ...
            # check the satellite coverage for each ground station
            for ground_station_id in range(0, len(ground_station_llas)):
                ground_station_lla = ground_station_llas[ground_station_id]
                ground_station_cbf = ground_station_cbfs[ground_station_id]
                gs_covered = checkSatCoverGroundStation(satellite_cbf, ground_station_cbf, L)
                if gs_covered:
                    covered_gs_num += 1 
                    sat_gs_latency = computeLatency(satellite_cbf, ground_station_cbf)
                    # remember actual ground_station_id in G has a offset of satellite_num
                    edges.append((satellite_id, ground_station_id + satellite_num, sat_gs_latency))
